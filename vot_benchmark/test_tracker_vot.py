...
import sys
import os
PTH= "/home/devyhia/PY-GOTURN"
sys.path.insert(1, PTH)   # goturn module path
...
def main(proto, model):
    setproctitle.setproctitle('VBT_%s_%s' % (proto, model))
    args = {
            "prototxt": "%s/nets/%s.prototxt" % (PTH, proto),
            "model": "%s/%s.caffemodel" % (PTH, model),
            "gpuID": 0
    }
    do_train = False
